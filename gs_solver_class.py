...
class RRT_Tokamak(object):
    ...
    def _pts_in_lcfs(self):
        ...
        Returns:
            plasma_grid_arr (array): stores (R,Z) points within plasma
            count (int): the number of points within plasma
        """
        ...
        output = np.zeros((count,2))
        for i in range(count):
            output[i][0] = plasma_grid_full[i][0]
            output[i][1] = plasma_grid_full[i][1]
        return output
    ...
    def compute_psi(self):
        ...
        plasma_grid_arr = self._pts_in_lcfs()
